...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Please mention 1 person", event.threadID, event.messageID);
    else {
        var one  = mention;
        return makeImage({ one }).then(path => api.sendMessage({ body: event.mentions[mention].replace("@", ""), attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}
