module.exports.config = {
  ...
  envConfig: {
    "YOUTUBE_API": "AIzaSyBZuIv9MNYOCbGYEZm-9N0hrZKJMob-qT0"
  }
};
...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  try {
    ytdl.getInfo(handleReply.link[event.body - 1]).then(res => {
      let body = res.videoDetails.title;
      ytdl(handleReply.link[event.body - 1], { quality: '18' })  // '18' is the code for 360p resolution
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400)
            return api.sendMessage('The file could not be sent because it is larger than 25MB.', event.threadID, () =>
              unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
          else
            return api.sendMessage({ body: `${body}`, attachment: createReadStream(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`) }, event.threadID, () =>
              unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID)
        })
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (urlValid) {
    try {
      ...
      ytdl(args[0], { quality: '18' })  // '18' is the code for 360p resolution
        .pipe(createWriteStream(__dirname + `/cache/${id}.mp4`))
        .on("close", () => {
          if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400)
            return api.sendMessage('The file could not be sent because it is larger than 25MB.', event.threadID, () =>
              unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
          else
            return api.sendMessage({ attachment: createReadStream(__dirname + `/cache/${id}.mp4`) }, event.threadID, () =>
              unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID)
        })
        .on("error", (error) => api.sendMessage(`There was a problem while processing the request, error: \n${error}`, event.threadID, event.messageID));
    } catch {
    ...
    try {
      ...
      var body = `There are ${link.length} results matching your search keyword:\n\n${msg}Please reply to this message with the number of your choice.`
      
      return api.sendMessage({ attachment: imgthumnail, body: body }, event.threadID, (error, info) =>
      ...
      var body = `There are ${link.length} results matching your search keyword:\n\n${msg}Please reply to this message with the number you want to play.`
