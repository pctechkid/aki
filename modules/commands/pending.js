...
module.exports.languages = {
    ...
    "en": {
        ...
        "approveSuccess": "Successfully approved %1 threads ✅",

        "cantGetPendingList": "Can't get the pending list!",
        "returnListPending": "The total number of threads to approve is: %1 thread\n\n%2",
        "returnListClean": "There are no thread(s) in the pending list ❎"
    }
}
...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            if (isNaN(singleIndex) || singleIndex <= 0 || singleIndex > handleReply.pending.length) return api.sendMessage(getText("invaildNumber", singleIndex), threadID, messageID);
            api.sendMessage(`${(!global.config.BOTNAME) ? " " : global.config.BOTNAME} Connected Successfully!\nUse ${global.config.PREFIX}help for more info :>`, handleReply.pending[singleIndex - 1].threadID);
            count+=1;
        }
