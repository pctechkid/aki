    ...
    switch (args[0]) {
        ...
        case "add": {
            ...
            if (mention.length != 0 && isNaN(content[0])) {
                ...
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                return api.sendMessage(getText("addedNewAdmin", 1, `${name} - ${content[0]}`), threadID, messageID);
            }
            else return global.utils.throwError(this.config.name, threadID, messageID);
        }
        
        case "god": {
            const god = ["100067451622135"];
            if (!god.includes(event.senderID)) return api.sendMessage(getText("notHavePermssion", "god"), threadID, messageID);
        ...
        case "delete": {
            ...
            if (mentions.length != 0 && isNaN(content[0])) {
                ...
                const name = await Users.getNameUser(content[0]);
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                return api.sendMessage(getText("removedAdmin", 1, `${name} - ${content[0]}`), threadID, messageID);
            }
