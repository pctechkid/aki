...
module.exports.handleReply = async function ({ api, event, handleReply }) {
    ...
    try {
        ...
        return api.sendMessage({ 
            body: `Title: ${data.title}\nDuration: ${this.convertHMS(data.dur)}`,
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        try {
          ...
          try {
            ...
            var body = `There are ${link.length} results that matched "${keywordSearch}"\n\n${msg}\nPlease reply the number of your choice`
            return api.sendMessage({
              body: body
            ...
            return api.sendMessage('An error occurred, please try again\n' + e, event.threadID, event.messageID);
        }
    }
    }