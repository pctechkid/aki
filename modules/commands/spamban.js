...
module.exports.config = {
 ...
 version: "2.0.0",
 hasPermssion: 0,
 credits: "Aki Hayakawa", //fix get by  D-Jukie
...
module.exports.run = async function ({api, event})  {
 return api.sendMessage(`Automatically ban users if spam ${num} times within ${timee}s`, event.threadID, event.messageID);
};
module.exports.handleEvent = async function ({ Users, Threads, api, event})  {
 ...
 if ((global.client.autoban[senderID].timeStart + (timee*1000)) <= Date.now()) {
   ...
   if (global.client.autoban[senderID].number >= num) {
     ...
     data.banned = true;
     data.reason = `Bot spam ${num} times / ${timee}s` || null;
     ...
     api.sendMessage("ID: " + senderID + "\nName: " + dataUser.name + `\nReason: Bot spam ${num} times / ${timee}s\n\nReported to bot admins`, threadID,
   () => {
   var idad = global.config.ADMINBOT;
   for(let ad of idad) {
       api.sendMessage(`Spam violation ${num} times / ${timee}s\n\nName: ${dataUser.name} \nID: ${senderID}\nGroup ID: ${threadID} \nGroup Name: ${namethread} \nTime: ${timeDate}`, 
         ad);
   }
   })
   }
