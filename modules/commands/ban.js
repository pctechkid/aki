...
module.exports.run = async function({
    ...
    */
    ...
    if (args[0] == "view") {
        if (!args[1]) {
            ...
            for (let idtvw in dtwbox) {
                var name = (await api.getUserInfo(idtvw))[idtvw].name,
                    msg = "",
                    solan = 1;
    ...
    //◆━━━━━━━━━◆WARN◆━━━━━━━━━◆\\
        ...
        api.sendMessage({
            body: `Banned members ${arrayname.join(", ")} permanently leave the group for the reason: ${reason}`,
            mentions: arraytag
        }, threadID, messageID);
        fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
    }
