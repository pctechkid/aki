...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (input.length !== 2) {
    api.sendMessage(
      'To use this command, simply type “myquote [text] = [author]”, where [text] is the quote you want to save and [author] is the name of the person who said it. For example, to save the quote "The best and most beautiful things in the world cannot be seen or even touched - they must be felt with the heart" by Helen Keller, you would type “myquote The best and most beautiful things in the world cannot be seen or even touched - they must be felt with the heart = Helen Keller.”',
      threadID,
      messageID
    );
    return;
  ...
  if (words.length > 25) {
    api.sendMessage(
      "The quote text must be 25 words or less only.",
      threadID,
      messageID
    );
    return;
  ...
  try {
    ...
    const randomBackground =
      backgroundLinks[Math.floor(Math.random() * backgroundLinks.length)];
    ...
    lines.forEach((line, index) => {
      if (lines.length === 1) {
        // Only one line, add double quotes at the beginning and end
        ctx.fillText(`"${line}"`, canvas.width / 2, textY + index * lineHeight);
      } else if (index === 0) {
        // First line, add double quotes at the beginning
        ctx.fillText(`"${line}`, canvas.width / 2, textY + index * lineHeight);
      } else if (index === lines.length - 1) {
        // Last line, add double quotes at the end
        ctx.fillText(`${line}"`, canvas.width / 2, textY + index * lineHeight);
      } else {
        // Not the first or last line, no quotes
        ctx.fillText(`${line}`, canvas.width / 2, textY + index * lineHeight);
      }
    });
