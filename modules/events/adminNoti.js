      ...
      switch (logMessageType) {
          case "log:thread-admins": {
              if (logMessageData.ADMIN_EVENT === "add_admin") {
                  Users.getNameUser(logMessageData.TARGET_ID)
                      .then(targetUserName => {
                          api.sendMessage(`${targetUserName} became a Group Admin`, threadID);
                      })
                      .catch(error => {
                  ...
                  Users.getNameUser(logMessageData.TARGET_ID)
                      .then(targetUserName => {
                          api.sendMessage(`Removed admin privileges for ${targetUserName}`, threadID);
                      })
                      .catch(error => {
                          console.error("Error getting user name:", error);
                      });
          ...
          case "log:user-nickname": {
              ...
              if (participant_id && nickname) {
                  ...
                  api.sendMessage(`Updated nickname for ${participantName} into ${formattedNickname}`, threadID);
              }
              break;
          }
          case "log:thread-name": {
              dataThread.threadName = logMessageData.name || null;
              api.sendMessage(`${(dataThread.threadName) ? `Updated Group Name to ${dataThread.threadName}` : 'Cleared the Group Name'}.`, threadID);
              break;
          }
          ...
          case "log:thread-call": {
              if (logMessageData.event === "group_call_started") {
                  ...
                  api.sendMessage(`Started a ${(logMessageData.video) ? 'Video ' : ''}Call.`, threadID);
                  ...
                  api.sendMessage(`${(logMessageData.video) ? 'Video' : ''} Call has ended. Call duration: ${timeFormat}`, threadID);
          ...
          case "log:thread-color": {
              ...
              if (global.configModule[this.config.name].sendNoti) {
                  api.sendMessage(`${logMessageBody.replace("Theme", "color")}`, threadID, async (error, info) => {
                      if (global.configModule[this.config.name].autoUnsend) {
                          await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                          return api.unsendMessage(info.messageID);
                      }
