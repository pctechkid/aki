...
module.exports.run = async function ({ api, event, Users }) {
  ...
  //   api.changeNickname(`ᴀᴋi`, threadID, api.getCurrentUserID());
    return api.sendMessage("", event.threadID, () => api.sendMessage({ body: `Connected successfully!\nThank you for using my bot. Have fun!`}, threadID));
  }
  else {
    try {
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        abx.push(fs.createReadStream(__dirname + `/cache/join/${o}.gif`))
      }
      memLength.sort((a, b) => a - b);
      (typeof threadData.customJoin == "undefined") ? msg = `Welcome {name} to the group {threadName}. {type} are the group's {soThanhVien}${suffix} member. Enjoy!` : msg = threadData.customJoin;
      ...
      for (let ii = 0; ii < parseInt(id.length); ii++) {
        fs.unlinkSync(__dirname + `/cache/join/${ii}.gif`)
      }
